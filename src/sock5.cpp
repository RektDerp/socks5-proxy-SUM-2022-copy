...
{
	proxy::ConfigReader::getInstance().getValue("Auth", _authFlag);
}
...
bool Socks5::auth()
{
	...
	*/
	...
	bool success = proxy::ConfigReader::getInstance().hasUser(_username, passString);
	if (!success)
		log(proxy::ERROR_LOG) << "Wrong credentials: " << _username << ":" << passString;
	bvec response;
...
std::string Socks5::readAddress(unsigned char atyp)
{
	...
	sendErrorResponse(proxy::ADDR_TYPE_NOT_SUPP);
	return {};
}
